...
def create_celeba_hq(h5_filename, celeba_dir, delta_dir, num_threads=4, num_tasks=100):
    ...
    def process_func(idx):
        ...
        with open(os.path.join(celeba_dir, 'Anno', 'identity_CelebA.txt'), 'rt') as file2:
            iname = [[value for value in line.split()[0]] for line in file2.readlines()[:]]
            identities = [[value for value in line.split()[1:]] for line in file2.readlines()[:]]
            identities = np.float32(identities).reshape(-1, 1)
        ...
        # vis = Visdom(port='10001', env='identity')
        # IMG = vis.images(img[0], 'img0', opts=dict(caption='img0', title='img0'))
        # IMG1 = vis.images(img[1], 'img1', opts=dict(caption='img1', title='img1'))
        # IMG2 = vis.images(img[2], 'img2', opts=dict(caption='img2', title='img2'))
        # DT = vis.images(delta[0], 'delta', opts=dict(caption='delta', title='delta'))
        # ID = vis.images(img+delta[0], 'img+delta', opts=dict(caption='img+delta', title='img+delta'))
        ##############################################

        # Apply delta image.
        img = img + delta     # img=grey scale, img+delta=RGB
        
        # Verify MD5.
        md5 = hashlib.md5()
        md5.update(img.tobytes())
        # assert md5.hexdigest() == fields['final_md5'][idx]  # disable md5 verify
        return idx, img, orig_file

    print('Creating %s' % h5_filename)
    # h5 = HDF5Exporter(h5_filename, 1024, 3)
    # f = open('idx_fname.txt', 'w')
    image_path = './datasets/CelebA-HD/'
    with ThreadPool(num_threads) as pool:
        print('%d / %d\r' % (0, len(fields['idx'])))
        for idx, img, orig_file in pool.process_items_concurrently(fields['idx'], process_func=process_func, max_items_in_flight=num_tasks):
            img = np.transpose(img, (1,2,0))
            # imageio.imwrite(image_path+str(idx)+'_'+orig_file, img)
            scipy.misc.imsave(image_path+str(idx)+'_'+orig_file, img)
            # h5.add_images(img[np.newaxis])
            # f.write(orig_file)
            print('%d / %d  %s\r' % (idx + 1, len(fields['idx']), orig_file))
