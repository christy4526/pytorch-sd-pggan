...
class GDropLayer(nn.Module):
    ...
    def forward(self, x, deterministic=False):
        ...
        # [x.size(axis) for axis in self.axes]
        if self.mode == 'drop':
            p = 1 - self.strength
        ...
        # rnd = Variable(torch.from_numpy(rnd).float())
        rnd = torch.from_numpy(rnd).float()
        # rnd = Variable(rnd.type(x.data.type()))
        if x.is_cuda:
            rnd = rnd.cuda()
        return x * rnd
...
class LayerNormLayer(nn.Module):
    ...
    def __init__(self, incoming, eps=1e-4):
        ...
        self.incoming = incoming
        self.eps = eps
        self.gain = torch.nn.parameter(torch.FloatTensor([1.0]), requires_grad=True)
        self.bias = None
...
class GSelectLayer(nn.Module):
    ...
    def forward(self, x, y=None, cur_level=None, insert_y_at=None):
        ...
        out = {}
        if DEBUG:
            print('GSelect1 G: level=%s, size=%s' % ('in', x.size()))
        for level in range(_from, _to, _step):
            ...
            if DEBUG:
                print('GSelect2 G: level=%d, size=%s' % (level, x.size()))
...
class DSelectLayer(nn.Module):
    ...
    def forward(self, x, y=None, cur_level=None, insert_y_at=None):
        ...
        if DEBUG:
            print('DSelect1 D: level=%s, size=%s, max_level=%s, min_level=%s' % ('in', x.size(), max_level, min_level))
        ...
        for level in range(_from, _to, _step):
            ...
            if DEBUG:
                print('DSelect2 D: level=%d, size=%s' % (level, x.size()))
        return x
