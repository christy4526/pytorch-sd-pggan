...
def G_conv(incoming, in_channels, out_channels, kernel_size, padding, nonlinearity,
            init, param=None, to_sequential=True, use_wscale=True, use_batchnorm=False,
            use_pixelnorm=True, use_cuda=False):
    layers = incoming
...
def NINLayer(incoming, in_channels, out_channels, nonlinearity, init, param=None,
    ...
    layers = incoming
    layers += [nn.Conv2d(in_channels=in_channels, out_channels=out_channels,
                        kernel_size=1, stride=1, padding=0)]  # NINLayer in lasagne
    he_init(layers[-1], init, param)  # init layers
    if use_wscale:
        layers += [WScaleLayer(layers[-1])]
...
class Generator(nn.Module):
    def __init__(self, 
        ...
        layers = G_conv(layers, latent_size, self.get_nf(1), 4, 3, act, iact, 
                        negative_slope, False, self.use_wscale, self.use_batchnorm, 
                        self.use_pixelnorm) 
        ...
        lods.append(net)
        nins.append(NINLayer([], self.get_nf(1), self.num_channels, output_act, 
                    output_iact, None, True, self.use_wscale))  # to_rgb layer

        for I in range(2, R):  # following blocks
            ic, oc = self.get_nf(I-1), self.get_nf(I)
            # TODO: change Upsample to interpolate
            # layers = [F.interpolate(scale_factor=2, mode='nearest')]  # upsample
            layers = [nn.Upsample(scale_factor=2, mode='nearest')]  # upsample
            layers = G_conv(layers, ic, oc, 3, 1, act, iact, negative_slope, False, 
                            self.use_wscale, self.use_batchnorm, self.use_pixelnorm)
            net = G_conv(layers, oc, oc, 3, 1, act, iact, negative_slope, True, 
                        self.use_wscale, self.use_batchnorm, self.use_pixelnorm)
            lods.append(net)
            nins.append(NINLayer([], oc, self.num_channels, output_act, output_iact, 
                                None, True, self.use_wscale))  # to_rgb layer
    ...
    def forward(self, x1, x2, y=None, cur_level=None, insert_y_at=None):
        output1 = self.output_layer(x1, y, cur_level, insert_y_at)
        output2 = self.output_layer(x2, y, cur_level, insert_y_at)
        return output1, output2



def D_conv(incoming, in_channels, out_channels, kernel_size, padding, nonlinearity, 
            init, param=None, to_sequential=True, use_wscale=True, use_gdrop=True, 
            use_layernorm=False, gdrop_param=dict()):
    layers = incoming
    if use_gdrop:
...
class Discriminator(nn.Module):
    def __init__(self, 
        ...
        nins.append(NINLayer([], self.num_channels, self.get_nf(R-1), act, iact, 
        ...
        for I in range(R-1, 1, -1):
            ...
            nin = []
            nin = NINLayer(nin, self.num_channels, oc, act, iact, negative_slope, 
                            True, self.use_wscale)
            nins.append(nin)
        ...
        lods.append(NINLayer(net, self.get_nf(0), oc, output_act, output_iact,
                            None, True, self.use_wscale))
    ...
    def forward(self, x1, x2, y=None, cur_level=None, insert_y_at=None, gdrop_strength=0.0):
