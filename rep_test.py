...
class PGGAN():
    ...
    def compute_D_loss(self):
        ...
        d_sd_loss = self.BCELoss(self.d_real, self.real_label) + self.BCELoss(self.d_fake, self.fake_label)
    ...
    def forward_D(self, cur_level, detach=True):
        ...
        # print(self.fake1[0]*127.5+1.0)
        vissum.image2d('fake1-x'+str(cur_level),'fake1',
                self.fake1[0],nrow=cur_level)
        vissum.image2d('fake2-x'+str(cur_level),'fake2',
                self.fake2[0],nrow=cur_level)
        vissum.image2d('real1-x'+str(cur_level),'real1',
                self.real1[0],nrow=cur_level)
        vissum.image2d('real2-x'+str(cur_level),'real2',
                self.real2[0],nrow=cur_level)

        strength = self.compute_noise_strength()
        self.d_real = self.D(self.real1, self.real2, cur_level=cur_level, gdrop_strength=strength)
        self.d_fake = self.D(self.fake1.detach(), self.fake2.detach(), cur_level=cur_level)
        ...
        for it in range(from_it, total_it):
            ...
            if (it % self.opts['sample_freq'] == 0) or it == total_it-1:
                ...
                imageio.imwrite(os.path.join(self.opts['sample_dir'],
                                    'fake1_%dx%d-%s-%s.png' % (cur_resol, cur_resol, phase, str(it).zfill(6))), samples1)
                imageio.imwrite(os.path.join(self.opts['sample_dir'],
                                    'fake2_%dx%d-%s-%s.png' % (cur_resol, cur_resol, phase, str(it).zfill(6))), samples2)
            ...
            if it == total_it:
                self.vissum.image2d('save_fake1_'+str(cur_resol), self.fake1[0])
                self.vissum.image2d('save_fake2_'+str(cur_resol), self.fake2[0])
            self.vissum.save()
...
if __name__ == '__main__':
    ...
    data = pairCelebA(FG.celeba_dir, FG.img_dir)
